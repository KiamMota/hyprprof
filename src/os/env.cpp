...
core::PackageManagerEnum os::detect_package_manager() {
    ...
    if (pm == "pacman")
        return core::PackageManagerEnum::PACMAN;
    if (pm == "apt")
        return core::PackageManagerEnum::APT;
    if (pm == "dnf")
        return core::PackageManagerEnum::DNF;
    if (pm == "yum")
        return core::PackageManagerEnum::YUM;
    if (pm == "zypper")
        return core::PackageManagerEnum::ZYPPER;
    if (pm == "apk")
        return core::PackageManagerEnum::APK;
    if (pm == "xbps")
        return core::PackageManagerEnum::XBPS;
...
std::string os::detect_user_name() { return getenv("USER"); }
